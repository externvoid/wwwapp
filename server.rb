...
require 'date'
Dir.glob('./helper/*.rb').each do |e| require e.gsub(/\.rb/, '') end
# require './req'
$stdout.sync = true
#$stderr.sync = true
before do
  ...
  @db.execute  "attach database 'n225Hist.db' as sub;"
  @cdb.execute "attach database 'n225Hist.db' as sub;"
  sql1 = "select name from sqlite_master where type='table' " + 
    "order by name desc limit 1;"
  codeTbl = @cdb.execute(sql1).first.first
  sql2 = "select code, company, exchange, marketcap, feature, category from '#{codeTbl}' " + 
    "union " +
    "select code, company, '---', '---', '---', '---' from sub.'n225Tbl';"
...
get '/chart/:code' do
  content_type :html
  @code = params[:code]
  @company = @codeAr.select do |e| e.first == @code end.flatten.join ':'
  # @company = @codeAr.select do |e| e.first == @code end.flatten[1]
  puts @company
  @test2 =  {'foo': 3, 'bar': 4}
  @test = getjson
